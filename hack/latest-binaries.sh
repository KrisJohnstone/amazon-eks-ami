...
LATEST_BINARIES=$(aws s3api list-objects-v2 --bucket amazon-eks --prefix "${MINOR_VERSION}" --query 'Contents[*].[Key]' --output text | grep "${PLATFORM}" | cut -d'/' -f-2 | sort -Vru | head -n1)
